...
import {
  type ColMeta, type SalesFile, type RegionTotalCol, type ViewCol,
  isRT, toNum, normalizeItemName, cellVal, rowTotal,
  normColHeader, COMPANY_KW, detectCompanyCol, detectItemNameCol, detectPriceCol,
  parseStockFile, buildMergedFile,
...
table{border-collapse:collapse;width:100%}
  ...
  const exportTableToPDF = useCallback(async () => {
    ...
    clone.querySelectorAll('[data-export="omit"]').forEach(el => el.remove());
    // Remove open dropdown menus / popovers (but NOT export-safe elements like category badges)
    ...
    clone.querySelectorAll('th, td').forEach(c => {
      ...
      if (pos === 'sticky') el.style.position = 'relative';
    });
  ...
  // كشف عمودَي السعر واسم الايتم عبر نفس دوال stockParser (مصدر واحد للحقيقة —
  // كان هنا نسخة ثانية من المنطق تختلف قليلاً عن التي يستعملها الدمج والخادم).
  const priceCol = useMemo(() => activeFile ? detectPriceCol(activeFile) : '', [activeFile]);
  const itemNameCol = useMemo(() => activeFile ? detectItemNameCol(activeFile) : '', [activeFile]);
  ...
  const redCellCount = useMemo(() => {
    ...
    // When 'all' regions: use raw areaCols; when specific region+warehouses: use displayCols (individual cols)
    let cols: ColMeta[];
    if (regionFilter === 'all') {
  ...
  const pageRows = useMemo(() => {
    ...
    if (shortageOnlyMode) {
      const T = Math.max(0, shortageThreshold || 0);
      // In multi-region mode displayCols are RegionTotalCols (isRT=true) so we
      // check individual warehouse cols instead.
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  {selectedRegions.map(region => {
                    ...
                    const regionCols = sortAFirst
                      ...
                      : rawRegionCols;
                    const visibleCols = warehouseKeys.size > 0 ? regionCols.filter(ac => warehouseKeys.has(ac.key)) : regionCols;
                    const T = Math.max(0, shortageThreshold || 0);
                    const visibleRows = shortageOnlyMode
                      ? pageRows.filter(row => visibleCols.some(ac => { const v = toNum(row[ac.key] ?? ''); return v === 0 || (T > 0 && v > 0 && v < T); }))
                      : pageRows;
                                    ...
                                    return (
                                      <tr key={idx} style={{ borderBottom: '1px solid var(--c-border-light)' }}
                                        ...
                                        {visibleCols.map(col => {
                                          ...
                                          const isAbove = shortageOnlyMode && v > 0 && (T === 0 || v >= T);
                                          const cat = getCategory(col.region, col.label);
                                          const dim = focusCategoryA && cat !== 'A';
                                          const focusA = focusCategoryA && cat === 'A';
