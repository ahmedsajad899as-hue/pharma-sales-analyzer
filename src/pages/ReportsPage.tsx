...
import RepSelectOptions, { type GroupableRep, groupRepsByTeam } from '../components/RepSelectOptions';
...
/* لوحة ألوان هادئة لتمييز كل شركة في رأس عمود «كل المندوبين العلميين». مرّت
   بمحاولتين: باهتة جداً فبدت بلا فرق فعلي داخل إكسل، ثم مشبَّعة جداً فبدت
   صارخة/غير مريحة — هذه نسخة وسط بينهما (تشبّع 26-28% لا 20% ولا 40%). النطاق
   محصور عمداً بين الأزرق والبنفسجي (200°-320°) بلا أي درجة من الأحمر/الأصفر/
   الأخضر، كي لا تتصادم ألوان الشركات مع دلالتي أحمر=سالب/أخضر=موجب المستعملتين
   لصافي المبيع بنفس الجدول، ولا تبدو الألوان مشوَّهة بصرياً. hex بدل hsl()
   مباشرة كي يُستعمل نفسه في تدرّج CSS على الشاشة وفي تعبئة ARGB بالإكسل. */
...
const companyColorPair = (index: number): [light: string, dark: string] => {
  const hue = 200 + ((index * 26) % 120); // أزرق ← بنفسجي فقط، بعيداً عن الأحمر/الأصفر/الأخضر
  return [hslToHex(hue, 27, 63), hslToHex(hue, 23, 46)];
};
...
const styleSheet = (ws: XLSX.WorkSheet, aoa: any[][], colWidths?: number[], headerRows = 1) => {
  ...
  const headerRow = aoa[headerRows - 1] ?? [];
...
export default function ReportsPage({ activeFileIds, onNavigate }: Props) {
  ...
  // تفاصيل مكتب/مذخر لكل المندوبين العلميين دفعة واحدة — يفتحها زر «كل المندوبين العلميين»
  const [showAllRepsWarehouseModal, setShowAllRepsWarehouseModal] = useState(false);
  const [allRepsWarehouseLoading, setAllRepsWarehouseLoading]     = useState(false);
...
// Always load scientific reps — doctor-visit reports don't require uploaded Excel files
  ...
  // ملخص صافي المبيع + طلبيات المكتب/المذخر لكل المندوبين العلميين المرئيين
  // للمستخدم دفعة واحدة — القائمة الثانوية «كل المندوبين العلميين».
  const loadAllRepsWarehouseSummary = async () => {
    if (sciReps.length === 0) { setAllRepsWarehouseData([]); return; }
    setAllRepsWarehouseLoading(true); setAllRepsWarehouseData(null); setExpandedWarehouseRepId(null);
    try {
      const params = new URLSearchParams();
      params.set('repIds', sciReps.map(r => r.id).join(','));
      if (fromDate) params.set('startDate', fromDate);
      if (toDate)   params.set('endDate', toDate);
      if (activeFileIds.length > 0) params.set('fileIds', activeFileIds.join(','));
      const res = await fetch(`/api/scientific-reps/warehouse-summary?${params}`, { headers: authH() });
      const json = await res.json();
      if (!res.ok) throw new Error(json.message || t.reports.errLoad);
      setAllRepsWarehouseData(json.data ?? []);
    } catch (err: any) { setError(err.message); setAllRepsWarehouseData([]); }
    finally { setAllRepsWarehouseLoading(false); }
  };
  ...
  // يرتّب مندوبي القائمة الثانوية حسب الشركة (نفس تجميع قائمة اختيار المندوب
  // RepSelectOptions — قائد الفريق ثم مندوبوه، فالباقون، لكل شركة بالترتيب
  ...
  // محوري: صف لكل مذخر (+ صف «مكتب» للطلبيات بلا مذخر محدَّد) وعمود لكل
  // مندوب علمي — بنفس شكل شيت الإكسل المرجعي.
  const buildWarehousePivot = (data: RepWarehouseSummary[]) => {
    const warehouseNames = new Set<string>();
    data.forEach(r => r.warehouses.forEach(w => warehouseNames.add(w.name)));
  ...
  // بلا أصفار ظاهرة بالخلايا الفردية (فراغ بدلها)، مرتَّب حسب الشركة، ورأس
  ...
  const exportWarehouseSummaryToExcel = () => {
    ...
    if (warehouseViewMode === 'pivot') {
      ...
      const nameCol = 0;
      const totalCol1 = 1 + cols.length;
      const totalCol2 = 2 + cols.length;
      ...
      ws['!merges'] = [
        { s: { r: 0, c: nameCol },   e: { r: 2, c: nameCol } },
        { s: { r: 0, c: totalCol1 }, e: { r: 2, c: totalCol1 } },
    ...
    <div className="page" ref={pageRootRef} tabIndex={-1} style={{ outline: 'none' }}>
      ...
      <div className="filter-card">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {mode === 'scientific' && (
            ...
            ><Icon name="pharmacy" size={13} /> كل المندوبين العلميين</button>
          )}
      ...
      {mode === 'overall' && overallSales && (() => {
        ...
        const filterTaggedRows = (byAreaItem: AreaItemRow[], byItem: BreakdownRow[]): AreaItemRow[] => {
          const companyItemNames = tagCompanyNorms.length > 0
        ...
        const aggAreas = (rows: AreaItemRow[]): BreakdownRow[] => {
        ...
        const aggItems = (rows: AreaItemRow[], byItem: BreakdownRow[]): BreakdownRow[] => {
          const itemInfoMap = new Map(byItem.map(r => [r.name, r]));
          const map = new Map<string, BreakdownRow>();
          for (const r of rows) {
            if (!map.has(r.itemName)) {
              const info = itemInfoMap.get(r.itemName);
              map.set(r.itemName, { name: r.itemName, totalQty: 0, totalValue: 0, companyName: info?.companyName });
            }
        ...
        const aggCompanies = (rows: AreaItemRow[], byItem: BreakdownRow[]): BreakdownRow[] => {
          ...
          const map = new Map<string, BreakdownRow>();
          for (const r of rows) {
            const co = itemInfoMap.get(r.itemName)?.companyName; if (!co) continue;
            if (!map.has(co)) map.set(co, { name: co, totalQty: 0, totalValue: 0 });
            const row = map.get(co)!; row.totalQty += r.totalQty; row.totalValue += r.totalValue;
          }
          return [...map.values()].sort((a, b) => b.totalValue - a.totalValue);
        };
        // Single-text helpers (text mode only)
        const buildItemsFromArea = (byAreaItem: AreaItemRow[]): BreakdownRow[] => {
          ...
          return aggItems(filtered, overallSales.byItem);
        };
        const buildAreasFromItem = (byAreaItem: AreaItemRow[]): BreakdownRow[] => {
          ...
          return aggAreas(filtered);
        };

        // Compute filtered rows once for each dataset
        const salesTaggedRows = hasTags ? filterTaggedRows(overallSales.byAreaItem, overallSales.byItem) : [];
        const retTaggedRows   = hasTags ? filterTaggedRows(overallReturns?.byAreaItem ?? [], overallReturns?.byItem ?? []) : [];

        // Area tab filtering
        const salesAreasFiltered = hasTags
          ? aggAreas(salesTaggedRows)
          : (overallTab === 'area' && itemMatch && !areaMatch)
            ? buildAreasFromItem(overallSales.byAreaItem)
            : (overallTab === 'area' && companyMatch && !areaMatch)
              ? overallSales.byArea
              : filterRowsByText(overallSales.byArea);
        const retAreasFiltered = hasTags
          ? aggAreas(retTaggedRows)
          : (overallTab === 'area' && itemMatch && !areaMatch)
            ? buildAreasFromItem(overallReturns?.byAreaItem ?? [])
            : filterRowsByText(overallReturns?.byArea ?? []);

        // Item tab filtering
        const salesItemsFiltered = hasTags
          ? aggItems(salesTaggedRows, overallSales.byItem)
          : (overallTab === 'item' && areaMatch && !itemMatch)
            ? buildItemsFromArea(overallSales.byAreaItem)
            : filterRowsByText(overallSales.byItem);
        const retItemsFiltered = hasTags
          ? aggItems(retTaggedRows, overallReturns?.byItem ?? [])
          : (overallTab === 'item' && areaMatch && !itemMatch)
            ? buildItemsFromArea(overallReturns?.byAreaItem ?? [])
            : filterRowsByText(overallReturns?.byItem ?? []);

        // Company tab filtering
        const salesCompanyFiltered = hasTags
          ? aggCompanies(salesTaggedRows, overallSales.byItem)
          : filterRowsByText(overallSales.byCompany);
        const retCompanyFiltered = hasTags
        ...
        // Items: exclude contributions from excluded areas AND hide excluded items (incl. company-implied)
        const crossItems = (rows: BreakdownRow[], bai: AreaItemRow[]): BreakdownRow[] => {
          if (excAreaNames.size === 0 && allExcItemNames.size === 0) return rows;
          // Remove items that are explicitly or implicitly (via company) excluded
          const visibleRows = allExcItemNames.size > 0 ? rows.filter(r => !allExcItemNames.has(r.name)) : rows;
          if (excAreaNames.size === 0) return visibleRows;
          // Rebuild totals subtracting excluded area contributions
        ...
        // Companies: exclude contributions from excluded items, excluded areas, and hide excluded companies
