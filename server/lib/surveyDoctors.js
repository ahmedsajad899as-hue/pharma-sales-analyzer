...
// CRM/ملفات خارجية تكتب اسم الطبيب أحياناً "عيادة الدكتور فلان" أو "عيادة
// الدكتورة فلانة" (مؤنث) بدل "فلان/فلانة" وحدها — لو تُرك كما هو، لا يطابق
// السجل الصافي الموجود أصلاً. "الدكتورة" كانت تفلت من الصيغة القديمة لأن
// "الدكتور" وحدها تشترط مسافة تالية مباشرة، والحرف التالي فعلياً "ة" لا مسافة.
const DOCTOR_PREFIX_RE = /^\s*(عيادة\s+)?(الدكتورة|الدكتور|دكتورة|دكتور|د\.?)\s+/i;
// بعض ملفات CRM تلصق تفصيلاً إضافياً (منطقة/حي) بعد اسم الطبيب بشرطة بلا فاصل
// حقيقي («اسماء طه -ابو غريب») — نأخذ الاسم فقط. "/" مستثناة عمداً (قد تكون
// جزءاً من الاسم نفسه لا فاصلاً)؛ leadingSegment في doctor-visits-import.js
// يعالج "/" على مستوى الاستخراج المبكر لعمود client كاملاً.
const TRAILING_DASH_RE = /^(.+?)\s*[-–]\s*\S.*$/;
export function cleanDoctorName(name) {
  let s = String(name ?? '').trim();
  for (let i = 0; i < 3 && DOCTOR_PREFIX_RE.test(s); i++) s = s.replace(DOCTOR_PREFIX_RE, '').trim();
  const m = s.match(TRAILING_DASH_RE);
  if (m) s = m[1].trim();
  return s;
}
