...
export async function buildVisitOverlay(scope, dateFilter) {
  ...
  const visits = await prisma.doctorVisit.findMany({
    ...
    select: {
      id: true, visitDate: true, feedback: true, notes: true, itemName: true, geoCorrect: true,
  ...
  for (const v of visits) {
    ...
    const entry = { id: v.id, visitDate: v.visitDate, feedback: v.feedback, notes: v.notes, item, geoCorrect: v.geoCorrect };
