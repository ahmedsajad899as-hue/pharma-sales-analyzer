...
// حراسة برمجية بجانب تعليمة البرومبت — لا نثق بامتثال النموذج وحده: كلمة عامة
// («صيدلية»/«الصيدلية»/بلا اسم مخصّص) ليست اسم صيدلية، وردَّت فعلياً رغم
// التعليمة الصريحة. نرفضها هنا مهما التزم النموذج بالتعليمة أم لا.
const GENERIC_PHARMACY_NAMES = new Set(['صيدلية', 'الصيدلية', 'صيدليه', 'الصيدليه', 'pharmacy', 'the pharmacy']);
function isGenericPharmacyName(name) {
  const n = name.trim().toLowerCase();
  return GENERIC_PHARMACY_NAMES.has(n) || n.replace(/^(ال)/, '').trim() === 'صيدلية' || n.replace(/^(ال)/, '').trim() === 'صيدليه';
}
...
- خطأ حاسم يجب تفاديه: لا تُعد الكلمة العامة "صيدلية"/"الصيدلية"/"صيدليه" وحدها بلا اسم مخصّص يليها كأنها اسم صيدلية — هذه كلمة تصف نوع المكان فقط، ليست اسماً. مثال: نص يقول "تم توفيرها في الصيدلية" أو "متابعة توفير الصيدلية" بلا ذكر اسم محدد بعدها → لا يوجد اسم صيدلية هنا فعلياً، لا تُدرج هذه الزيارة إطلاقاً. اسم الصيدلية الحقيقي دائماً أخصّ من ذلك (اسم علم أو اسم شخص أو مكان محدد يلي الكلمة العامة، مثل "صيدلية الأمل" أو "صيدلية قمر الجامعة").
    ...
    try {
      ...
      if (Array.isArray(arr)) {
        ...
        for (const e of arr) {
          ...
          if (batchIds.has(id) && name && !isGenericPharmacyName(name)) results.set(id, name);
        }
      }
    } catch {
